# recommend.py -- Trump Prime Grillhouse
# DUAL MODE:
...
MENU_FILE            = os.path.join(BASE_DIR, "food", "TrumpMenu.json")
...
WAITER_STYLES = [
    ...
    "You are a polished grillhouse and wine-cellar guide. Keep the tone premium and hospitality-focused.",
]
...
def sanitize_item(item, source_title="You might also like"):
    ...
    return {
        ...
        "img":          str(item.get("img", "Images/Tomahawk.jpg")),
...
# ============================================================
# SECTION 4 -- LOCAL PAIRING HELPER
# ============================================================

def call_local_pairing(prompt, system, attempts=3, delay=1):
    return json.dumps({
        "title": "Chef's Recommendation",
        "description": "A premium grillhouse choice with enough presence for the table.",
        "pairings": [
            {"name": "Cabernet Sauvignon", "reason": "Structured tannin works well with char and richness."},
            {"name": "Garlic Lemon Calamari", "reason": "A bright starter keeps the table balanced."}
        ],
        "talkTrack": "This is a confident choice; I would build the table around it."
    })
...
def ai_pairing():
    ...
    user_prompt = (
        ...
        "3. 2 pairing suggestions from a premium grillhouse menu (food or drink) with a brief reason each\n\n"
    ...
    try:
        raw = call_local_pairing(user_prompt, system_prompt)
        if "```" in raw:
            raw = raw.split("```")[1]
        ...
        return jsonify({
            ...
            "pairings": [
                ...
                {"name": "Prime Rump 300g",   "reason": "A reliable steakhouse classic with a bold grill finish."}
            ],
...
if __name__ == "__main__":
    server_mode = os.environ.get("RECOMMEND_MODE") == "server"
    if not server_mode and not sys.stdin.isatty():
        try:
