# 🇬🇷 JOSH 11.0 - Trump Prime Grillhouse AI Assistant
...
├── food/                       ← place TrumpMenu.json here
...
```bash
cp /path/to/TrumpMenu.json josh_enterprise/food/TrumpMenu.json
```
...
## Multilingual Support (bilingual ↔ English)
...
```
User message (another language or English)
        │
        ▼
┌─────────────────────────────────────┐
│  Language Detection                 │
│  1. language Unicode char ratio        │  ← instant, no model
...
┌─────────────────────────────────────┐
│  Response Generation                │
│  lang="el" → localized template pools  │
...
The vocabulary fallback covers **200+ restaurant phrases** instantly with no model download. Most customer conversations work perfectly.
