"""
Language Handler - Guest Language to English (Fully Local)
=================================================
Strategy (in order):
  1. argostranslate  — offline neural MT (best quality, ~50 MB per lang pair)
  2. Vocabulary-based restaurant-domain translation dict (instant, no model)
  3. Passthrough     — if everything fails, treat as English

Language detection:
  - langdetect (fast, statistical)
  - Heuristic: if >20% chars are source-language Unicode block → "el"
...
_SOURCE_LANGUAGE_BLOCK = re.compile(r"[\u0370-\u03FF\u1F00-\u1FFF]")


def _source_language_char_ratio(text: str) -> float:
    if not text:
        return 0.0
    source_chars = len(_SOURCE_LANGUAGE_BLOCK.findall(text))
    return source_chars / len(text)
...
def detect_language(text: str) -> tuple[str, float]:
    ...
    """
    ratio = _source_language_char_ratio(text)

    # Strong heuristic signal
    if ratio > 0.25:
        return "el", min(0.5 + ratio, 0.99)
    if ratio > 0.05:
        # Possibly translated text with Latin punctuation
        try:
...
# ===========================================================================
# 2. Restaurant-domain source-language vocabulary
# ===========================================================================
...
GR_TO_EN_PHRASES: list[tuple[str, str]] = [
    ...
    ("vegan", "vegan"),  # same in source text often
    ("βίγκαν", "vegan"),
...
def _vocab_translate_el_to_en(text: str) -> tuple[str, bool]:
    """    Translate source text to English using the vocabulary table.
    Returns (translated_text, was_changed).
    """
    result = text.lower()
    changed = False
    # Sort by length descending so multi-word phrases match first
    for source_phrase, english in sorted(_GR_VOCAB.items(), key=lambda x: -len(x[0])):
        if source_phrase in result:
            result = result.replace(source_phrase, english)
            changed = True
...
# ===========================================================================
# 4. Optional response localization
# ===========================================================================

# Pre-built localized phrase map for common bot replies
...
def _translate_en_to_el(text: str) -> str:
    """
    Translate an English bot reply when localization is enabled.
...
class LanguageHandler:
    ...
    Also provides translate_back() for generating localized responses.
    """
