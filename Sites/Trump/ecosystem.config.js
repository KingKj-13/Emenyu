...
module.exports = {
  apps: [
    ...
    // "Carmella by Sir Gaspard" — the reference-implementation second real
    // tenant (own DB, own persona, real staff accounts, native /Carmella
    // routing per AD-001 — not another nginx-rewrite copy of Demo's pattern).
    // See emenyu-carmella/ARCHITECTURE_DECISIONS.md.
    {
      name: 'emenuy-carmella-api',
      cwd: path.resolve(__dirname, '..', 'Carmella'),
      script: './server.js',
      exec_mode: 'fork',
      instances: 1,
      autorestart: true,
      ...
      min_uptime: process.env.PM2_MIN_UPTIME || '10s',
      max_restarts: Number(process.env.PM2_MAX_RESTARTS || 10),
      max_memory_restart: process.env.PM2_CARMELLA_MAX_MEMORY_RESTART || '512M',
      error_file: './logs/pm2/emenuy-carmella-api-error.log',
      out_file: './logs/pm2/emenuy-carmella-api-out.log',
      log_file: './logs/pm2/emenuy-carmella-api-combined.log',
      log_date_format: 'YYYY-MM-DD HH:mm:ss Z',
      merge_logs: false,
      time: true,
      env: {
