...
// Phase 5 (AI Concierge) — canned quick-action / suggestion-chip copy. These
// only decide what QUESTION gets sent to the existing chat() endpoint; the
// engine (aiService.chat -> Recommendation Engine V2) still decides the
// actual answer/candidates. No recommendation logic lives here.
...
// A small, non-exhaustive but sensible context->chip mapping. Removes chips
// that no longer make sense and surfaces ones that do, per the product
// examples (steak in cart -> wine/sauce/dessert/coffee), without trying to
// be a full NLU system for a presentation-layer nicety.
