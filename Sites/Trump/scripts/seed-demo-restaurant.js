...
const CATEGORIES = [
  {
    title: 'Starters',
    items: [
      { name: 'Beef Carpaccio', price: 125, description: 'Thinly sliced cured beef, parmesan, rocket, truffle oil.', allergens: 'Dairy' },
  ...
  },
  {
    ...
    items: [
      { name: 'Wagyu Steak 300g', price: 485, description: 'Full-blood Wagyu sirloin, chimichurri.', allergens: '', chefPick: true },
      ...
      { name: 'T-Bone 500g', price: 445, description: 'Charcoal grilled, served with jus.', allergens: '' },
    ]
  },
  {
    title: 'Seafood',
    items: [
      { name: 'Grilled Kingklip', price: 285, description: 'Line-caught, lemon butter.', allergens: 'Fish, Dairy' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Seafood Linguine', price: 310, description: 'Prawns, calamari, tomato & chilli.', allergens: 'Shellfish, Gluten, Dairy' },
    ]
  },
  {
    title: 'Sides',
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Malva Pudding', price: 95, description: 'Warm sponge, butterscotch sauce.', allergens: 'Gluten, Dairy, Eggs' },
...
const HISTORY_BASKETS = [
  ...
  { t: 'table6', w: 'Demo Waiter', cov: 4, days: 7, items: [{ n: 'Wagyu Steak 300g', q: 2 }, { n: 'Steakhouse Chips', q: 2 }, { n: 'Shiraz', q: 2 }] },
  ...
  { t: 'table5', w: 'Demo Waiter', cov: 2, days: 2, items: [{ n: 'Beef Fillet Pasta' }, { n: 'Merlot' }, { n: 'Malva Pudding' }] },
...
const LIVE_ORDERS = [
  { t: 'table2', w: 'Demo Waiter', cov: 2, items: [{ n: 'Wagyu Steak 300g' }, { n: 'Creamed Spinach' }, { n: 'Cabernet Sauvignon' }] },
...
async function main() {
  ...
  if (CLEAR) {
    if (!APPLY && process.argv.includes('--clear')) {
      // --clear runs immediately (no separate dry-run mode for it, matching seed-bundles.js).
    }
    const [items, cats, bundles, recs, tables, guests, orders, assignments] = await Promise.all([
      ...
      prisma.order.deleteMany({ where: { restaurantId: RESTAURANT_ID } }),
      prisma.waiterAssignment.deleteMany({ where: { restaurantId: RESTAURANT_ID } }),
    ]);
    console.log(`Cleared demo-steakhouse rows: ${items.count} items, ${cats.count} categories, ${bundles.count} bundles, ${recs.count} recs, ${tables.count} tables, ${guests.count} guests, ${orders.count} orders, ${assignments.count} assignments.`);
  ...
  const BUNDLES = [
    ...
    { slug: 'seafood', persona: 'The Seafood Lover', icon: '🦐', description: 'From the coast, line to plate.', accent: '#3a6e8f',
      items: [{ course: 'Drink', n: 'Chardonnay' }, { course: 'Starter', n: 'Calamari Tubes & Tentacles' }, { course: 'Main', n: 'Grilled Kingklip' }, { course: 'Dessert', n: 'Crème Brûlée' }] },
    ...
    { slug: 'pasta', persona: 'The Pasta Lover', icon: '🍝', description: 'Comfort, twirled to perfection.', accent: '#5c7a4f',
      items: [{ course: 'Drink', n: 'Sauvignon Blanc' }, { course: 'Starter', n: 'Beef Carpaccio' }, { course: 'Main', n: 'Beef Fillet Pasta' }, { course: 'Dessert', n: 'Malva Pudding' }] },
  ];
