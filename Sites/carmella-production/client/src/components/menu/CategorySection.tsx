...
export const CategorySection = memo(function CategorySection({
  ...
  return (
    <section className={styles.section} id={`section-${section.title.toLowerCase().replace(/\s+/g, '-')}`}>
      ...
      >
        <ChevronDown size={20} className={`${styles.chevron} ${collapsed ? styles.chevronCollapsed : ''}`} />
...
function SubSection({ title, items, onAddToCart, onItemClick, happyHourDiscounts }: {
  ...
  return (
    <div className={styles.subSection}>
      <button className={styles.subTitleRow} onClick={() => setCollapsed(c => !c)} aria-expanded={!collapsed}>
        <ChevronDown size={15} className={`${styles.chevron} ${collapsed ? styles.chevronCollapsed : ''}`} />
        <h3 className={styles.subTitle}>{title}</h3>
      </button>
      <div className={styles.collapseOuter} style={{ gridTemplateRows: collapsed ? '0fr' : '1fr' }}>
        <div className={styles.collapseInner}>
          <div className={styles.grid}>
            {items.map((item, i) => (
              <MenuCard
