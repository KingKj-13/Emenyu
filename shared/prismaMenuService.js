/**
 * Shared PrismaMenuService — works for all restaurants (greek, imli, al_pescatore, trump).
 * Pass { restaurantId } in the constructor. Requires DATABASE_URL in the root .env.
 */

const path = require('path');
const dotenv = require('dotenv');

// One level up from shared/ → project root
...
function flattenMenu(menuData = {}) {
  ...
  Object.entries(menuData || {}).forEach(([categoryTitle, categoryValue]) => {
    ...
    Object.entries(categoryValue).forEach(([subTitle, subValue]) => {
      if (subTitle === 'items' || subTitle === 'visible' || !subValue || typeof subValue !== 'object') return;
  ...
  async loadMenu() {
    return this.withPrisma(
      ...
      async prisma => {
        ...
        const menu = {};
        (byParent.get(0) || []).sort((l, r) => l.sortOrder - r.sortOrder).forEach(root => {
          ...
          (byParent.get(root.id) || []).sort((l, r) => l.sortOrder - r.sortOrder).forEach(sub => {
              ...
              data: {
                ...
                metadata: Object.fromEntries(
                  Object.entries(recommendation).filter(
                    ([key]) => !['description', 'items', 'visible', 'hidden'].includes(key)
                  )
                )
              }
            });
        ...
        for (const entry of list) {
          ...
          await prisma.featuredItem.create({
            data: {
              ...
              sortOrder: index,
              metadata: Object.fromEntries(
                Object.entries(entry).filter(([key]) => !['name', 'reason', 'visible', 'hidden'].includes(key))
              )
            }
          });
          index += 1;
...
module.exports = { PrismaMenuService, flattenMenu };
